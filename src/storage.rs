...
use crate::sign::{ANCHOR_FILE, Checkpoint};
...
impl Storage {
    ...
    pub fn flush(&mut self) {
        ...
        for block in &self.buffer {
            let line = serde_json::to_string(block).expect("[Lattice-d] Failed to serialize block");
            writeln!(writer, "{}", line).expect("[Lattice-d] Failed to write block");
        }
    ...
    pub fn append_log(&self, entry: &str) {
        ...
        let mut writer = BufWriter::new(file);
        writeln!(writer, "{}", entry).expect("[Lattice-d] Failed to write log entry");
    }
    ...
    pub fn last_block(&self) -> Option<Block> {
        ...
        let contents = fs::read_to_string(&p).expect("[Lattice-d] Failed to read chain file");
        contents
            .lines()
            .last()
    ...
    pub fn read_chain_blocks(&self) -> Vec<Block> {
        ...
        for seg in self.chain_segments() {
            ...
            let contents = fs::read_to_string(&seg).expect("[Lattice-d] Failed to read chain file");
    ...
    pub fn append_checkpoint(&self, cp: &Checkpoint) {
        ...
        let mut writer = BufWriter::new(file);
        let line = serde_json::to_string(cp).expect("[Lattice-d] Failed to serialize checkpoint");
        writeln!(writer, "{}", line).expect("[Lattice-d] Failed to write checkpoint");
        writer.flush().expect("[Lattice-d] Failed to flush writer");
    }
        ...
        contents
            ...
            .map(|(i, line)| {
                serde_json::from_str(line).unwrap_or_else(|_| {
                    panic!("[Lattice-d] Failed to parse checkpoint at line {}", i)
                })
            })
            .collect()
    }
...
mod tests {
    ...
    fn test_flush_writes_to_disk() {
        ...
        let contents = std::fs::read_to_string(s.path(CHAIN_FILE)).unwrap();
    ...
    fn test_read_chain_blocks_walks_backups_in_order() {
        let tmp = tempdir().unwrap();
        let s = Storage::with_dir(tmp.path());
        let mk = |idx: u64| serde_json::to_string(&dummy_block(idx, &"0".repeat(64))).unwrap();
        std::fs::write(
            s.path(&format!("{}.bak.2", CHAIN_FILE)),
            format!("{}\n", mk(1)),
        )
        .unwrap();
        std::fs::write(
            s.path(&format!("{}.bak.1", CHAIN_FILE)),
            format!("{}\n", mk(2)),
        )
        .unwrap();
